...
# Range oof disparity/inverse depth values
DISP_SCALING = 10
MIN_DISP = 0.01

def resize_like(inputs, ref):
	_, iH, iW, _ = inputs.get_shape().as_list()
	_, rH, rW, _ = ref.get_shape().as_list()
	if (iH == rH) and (iW == rW):
		return inputs
...
def pose_exp_net(tgt_image, src_image_stack, do_exp=True, is_training=True):
    ...
    with tf.variable_scope('pose_exp_net') as sc:
        ...
        with slim.arg_scope([slim.conv2d, slim.conv2d_transpose],
            ...
            return pose_avg, pose_final, [mask1, mask2, mask3, mask4], end_points



def disp_net(tgt_image, is_training=True):
	_, H, W, _ = tgt_image.get_shape().as_list()
	...
	with tf.variable_scope('depth_net') as sc:
		...
		with slim.arg_scope([slim.conv2d, slim.conv2d_transpose],
			...
			upcnv6 = slim.conv2d_transpose(icnv7, 512, [3, 3], stride = 2, scope = 'upconv6')
			#print(upcnv6)
			...
			upcnv5 = slim.conv2d_transpose(icnv6, 256, [3, 3], stride = 2, scope = 'upconv5')
			#print(upcnv5)
			...
			upcnv4 = slim.conv2d_transpose(icnv5, 128, [3, 3], stride = 2, scope = 'upconv4')
			#print(upcnv4)
			...
			disp1 = DISP_SCALING * slim.conv2d(i1_in, 1, [3, 3], stride = 1, 
